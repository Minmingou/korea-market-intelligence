...
# Mock과 동일하게 실제 KOSPI/KOSDAQ 공식 지수값 대신, 조회한 종목들의
# 시가총액 가중평균 등락률로 지수 등락을 추정한다. 공식 지수 산출 방식과
# 다른 자체 추정치임을 README에도 명시한다.
...
class KISClient(MarketDataClient):
    ...
    def _parse_stock(
        ...
        return RawStock(
            ...
            data_source="kis",
            fetched_at=now,
            # 20일 평균거래량/투자자별 순매수는 이 엔드포인트로 알 수 없어 N/A로 둔다.
            avg_volume_20d=None,
        ...
        for market, base_index_value in BASE_INDEX_VALUE.items():
            ...
            indices.append(
                RawMarketIndex(
                    ...
                    fetched_at=now,
                    # 종목별 투자자 순매수가 없어 지수 단위 합계도 만들 수 없다.
                    foreign_net_buy=None,
                    institution_net_buy=None,
                    individual_net_buy=None,
                )
